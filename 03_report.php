...
while($line = fgetcsv($fh, 8000)) {
    ...
    foreach($colIndex AS $gender => $cols) {
        $high[$line[1]][$y][$gender] = array();
        foreach($cols AS $age => $index) {
            $high[$line[1]][$y][$gender][$age] = $line[$index];
        }
    }
    $medium = $low = $high;
    for($y = 2020; $y < 2066; $y++) {
        ...
        for($age = 1; $age <= 100; $age ++) {
            $pAge = $age - 1;
            $high[$line[1]][$y]['m'][$age] = round($high[$line[1]][$py]['m'][$pAge] * (1 - $dr[$age]['m'][$drKey]));
            $high[$line[1]][$y]['f'][$age] = round($high[$line[1]][$py]['f'][$pAge] * (1 - $dr[$age]['f'][$drKey]));
            $medium[$line[1]][$y]['m'][$age] = round($medium[$line[1]][$py]['m'][$pAge] * (1 - $dr[$age]['m'][$drKey]));
            $medium[$line[1]][$y]['f'][$age] = round($medium[$line[1]][$py]['f'][$pAge] * (1 - $dr[$age]['f'][$drKey]));
            $low[$line[1]][$y]['m'][$age] = round($low[$line[1]][$py]['m'][$pAge] * (1 - $dr[$age]['m'][$drKey]));
            $low[$line[1]][$y]['f'][$age] = round($low[$line[1]][$py]['f'][$pAge] * (1 - $dr[$age]['f'][$drKey]));
        }
        ...
        for($age = 15; $age <= 49; $age ++) {
            ...
            $babyHigh += round($high[$line[1]][$y]['f'][$age] * ($birthHigh[$y][$brKey] / 1000));
            $babyMedium += round($medium[$line[1]][$y]['f'][$age] * ($birthMedium[$y][$brKey] / 1000));
            $babyLow += round($low[$line[1]][$y]['f'][$age] * ($birthLow[$y][$brKey] / 1000));
        }
        ...
        $age = 0;
        $high[$line[1]][$y]['m'][$age] = $babyHighM;
        $high[$line[1]][$y]['f'][$age] = $babyHigh - $babyHighM;
        if($high[$line[1]][$y]['f'][$age] < 0) {
            $high[$line[1]][$y]['f'][$age] = 0;
        }
        $medium[$line[1]][$y]['m'][$age] = $babyMediumM;
        $medium[$line[1]][$y]['f'][$age] = $babyMedium - $babyMediumM;
        if($medium[$line[1]][$y]['f'][$age] < 0) {
            $medium[$line[1]][$y]['f'][$age] = 0;
        }
        $low[$line[1]][$y]['m'][$age] = $babyLowM;
        $low[$line[1]][$y]['f'][$age] = $babyLow - $babyLowM;
        if($low[$line[1]][$y]['f'][$age] < 0) {
            $low[$line[1]][$y]['f'][$age] = 0;
        }
    }
}

$basePath = __DIR__ . '/reports/cunli';
foreach($high AS $cunliCode => $col1) {
    $targetPath = $basePath . '/' . $cunliCode;
...
foreach($medium AS $cunliCode => $col1) {
    $targetPath = $basePath . '/' . $cunliCode;
